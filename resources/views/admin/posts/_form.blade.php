    ...
    <div class="lg:col-span-2 space-y-5">
      ...
      <div class="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
        ...
        <input type="text" name="title"
               ...
               class="w-full px-4 py-3 border rounded-xl text-sm 
                      ...
                      @error('title') border-red-400 bg-red-50 @else border-gray-200 @enderror"/>
        @error('title')
      ...
      <div class="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
        ...
        <textarea name="content" id="content" rows="16"
                  ...
                  class="w-full px-4 py-3 border border-gray-200 rounded-xl 
                         text-sm focus:outline-none focus:ring-2 
                         focus:ring-primary-500 resize-y
                         @error('content') border-red-400 bg-red-50 @enderror">{{ old('content', $post?->content) }}</textarea>
        @error('content')
          <p class="mt-1.5 text-xs text-red-600">{{ $message }}</p>
        @enderror
      </div>
      ...
      <div class="bg-white rounded-2xl p-5 shadow-sm border border-gray-100">
        ...
        <div class="space-y-3">
          <button type="submit" name="status_action" value="published"
            ...
            {{ $post ? '💾 Simpan Perubahan' : '🚀 Publikasikan' }}
          </button>
          <button type="submit" name="status_action" value="draft"
                  class="w-full px-4 py-2.5 text-sm font-medium text-gray-600 
                         border border-gray-200 rounded-lg bg-white 
                         hover:bg-gray-50 transition-colors">
            💾 Simpan Draft
          </button>
          <a href="{{ route('admin.posts.index') }}"
             class="block text-center px-4 py-2.5 text-sm text-gray-400 
      ...
      <div class="bg-white rounded-2xl p-5 shadow-sm border border-gray-100">
        ...
        <select name="category_id"
                class="w-full px-4 py-2.5 border rounded-xl text-sm 
                       ...
                       @error('category_id') border-red-400 @else border-gray-200 @enderror">
